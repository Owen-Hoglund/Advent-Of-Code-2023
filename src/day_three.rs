...
pub fn day_three(file_content: String) {
    ...
    let valid_parts = part_validation(engine_schematic.clone(), parts.clone());
                ...
                match schematic.get(i) {
                    Some(row) => {
                        match row.get(j) {
                            ...
                            None => todo!("I think this should do nothing? line ~36"),
                        }
                    },
                    None => todo!("if it yields nothing move on ~36"),
                }
            }
        }
...
fn get_gear_ratio(parts: &Vec<PartNumber>) -> u32 {
    parts.iter().map(|x| x.number).fold(1, |acc, x| acc * x)
}
...
fn find_gears(schematic: &Vec<Vec<char>>) -> Vec<Gear> {
    ...
    for (y, row) in schematic.iter().enumerate() {
        for (x, c) in row.iter().enumerate() {
            if c == &'*' {
                gears.push(Gear { coordinates_y_x: (y, x), adjacent_parts: Vec::new(), gear_ratio:  0})
            }
        }
    }
    gears
...
fn get_border(schematic: &Vec<Vec<char>>, coordinates: (usize, usize, usize), border_width: u32,) -> Vec<char> {
    ...
    // take the entire cube, remove the parts that are included in the selection, and then flatten
    let y1 = cmp::max(0, coordinates.0 as i32 - border_width as i32) as usize;
    let y2 = coordinates.0 as usize + border_width as usize;
    let x1 = cmp::max(0, coordinates.1 as i32 - border_width as i32) as usize;
    let x2 = coordinates.2 as  usize + border_width as usize;
    for i in y1..=y2 {
        for j in x1..=x2 {
            match schematic.get(i) {
                Some(row) => {
...
fn numbers_in_row(row: Vec<char>, y: usize) -> Vec<PartNumber> {
    ...
    let mut coordinates: (usize, usize, usize) = (y, 0 as usize, 0 as usize);
